-module(token_keeper_claim_utils).
...
-export_type([decodable_authdata/0]).
...
-spec encode_authdata(decodable_authdata()) -> claims().
...
create_metadata(Claims, CompatOpts) ->
    ...
    token_keeper_utils:remap(genlib_map:compact(Metadata), maps:get(metadata_mappings, CompatOpts)).
